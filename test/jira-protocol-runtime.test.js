...
(async function () {
    ...
    try {
        ...
        assert.strictEqual(assetScopeScripts.length, 1);
        assert.strictEqual(assetScopeScripts[0], script,
            "Protocol re-authorization must use the same authoritative stored script scope as the selector.");
        ...
        assert.ok(seedSource.indexOf("SirkJiraAssetAql: objectType = Computer") >= 0,
            "Jira Asset Protocol must own its Computer Assets scope in script metadata rather than global integration settings.");
        ...
        console.log("Jira Asset Protocol lifecycle, script-scoped authoritative asset recheck, progress and protected PDF contract: OK");
    } finally {
