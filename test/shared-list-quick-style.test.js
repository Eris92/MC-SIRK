...
assert.ok(tree.indexOf('" sirk-shared-list-item"') >= 0 &&
    ...
    "Tree rows must receive the shared list classes directly while they are rendered.");
...
assert.ok(status.indexOf("sirk-shared-list-item") >= 0 &&
    ...
    "Status navigation must render the same shared list contract directly.");
assert.ok(catalog.indexOf("mc-catalog-results") >= 0 &&
    ...
    "Results must use the same shared list contract as roots and status rows.");
...
assert.ok(css.indexOf(".sirk-shared-list-item,.sirk-quick-command-categories>button,.sirk-quick-command-tree>button{display:grid;grid-template-columns:24px minmax(0,1fr);gap:8px") >= 0,
    "Approval, Commands, My Scripts and both Quick columns must share one 24 px icon grid.");
...
assert.ok(css.indexOf(".sirk-shared-list-item:hover") >= 0 &&
    css.indexOf(".sirk-quick-command-categories>button:hover") >= 0 &&
    css.indexOf(".sirk-shared-list-item.active") >= 0 &&
    css.indexOf(".sirk-quick-command-tree>button.active") >= 0,
    "Hover and selected states must have one static CSS owner for shared lists and Quick.");
...
assert.ok(desktopStyle >= 0 && sharedStyle > desktopStyle,
    "The final shared list stylesheet must load after Quick module CSS so it remains the visual owner.");

console.log("Direct shared-list rendering and single static style owner: OK");
