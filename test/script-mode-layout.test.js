...
assert.ok(source.indexOf(".mc-shared-page:is(.is-edit-mode,.is-multi-mode) .mc-shared-layout{grid-template-columns:var(--sirk-mode-primary-width,220px) calc(var(--sirk-mode-secondary-width,340px) + var(--sirk-actions-width) + var(--sirk-actions-column-gap))") >= 0,
    "Edit and Multi must preserve the captured first and second columns and add only the action area.");
assert.ok(source.indexOf(".mc-shared-page:is(.is-edit-mode,.is-multi-mode) .mc-shared-layout.is-collapsed{grid-template-columns:var(--sirk-primary-collapsed-track) calc(var(--sirk-mode-secondary-width,340px) + var(--sirk-actions-width) + var(--sirk-actions-column-gap))") >= 0,
...
assert.strictEqual(source.indexOf(".mc-shared-page-mycommands.is-multi-mode .mc-shared-layout"), -1,
    "Commands must not have a separate percentage-based Multi geometry.");
assert.strictEqual(source.indexOf("minmax(480px,52%)"), -1,
...
assert.ok(source.indexOf('page.classList.contains("is-edit-mode") ||') >= 0 &&
    source.indexOf('page.classList.contains("is-multi-mode")') >= 0,
