...
function item(id, assigned) { return { value: id, assetId: id, objectId: id, objectKey: "IT-" + id, objectType: "Laptop", hostname: "PC-" + id, manufacturer: "Vendor", model: "Model " + id, serialNumber: "SN-" + id, inventoryNumber: "INV-" + id, assignedToUser: assigned }; }
(async function () {
    ...
    try {
        ...
        var live = { "1001": "acc-1", "1002": "", "1003": "acc-1" }, puts = [], reads = [], forcedRefresh = 0;
        ...
        var service = factory.createJiraProtocolService({ context: context, jiraAssets: { listUsers: function () { return Promise.resolve({ items: [{ value: "acc-1", accountId: "acc-1", displayName: "User One", emailAddress: "one@example.invalid" }] }); }, listAssets: function (userValue, variable, force) { if (force) forcedRefresh++; return Promise.resolve({ items: userValue ? [item("1001", true), item("1003", true)] : [item("1001", true), item("1002", false), item("1003", true)], stale: false }); } }, requestJson: function (options) { if (/\/objecttype\/10\/attributes$/.test(options.url)) return Promise.resolve([{ id: "55", name: "Owner", editable: true, maximumCardinality: 1, defaultType: { name: "User" } }]); var match = /\/object\/(100[123])$/.exec(options.url); if (match && options.method === "GET") { reads.push(match[1]); return Promise.resolve({ id: match[1], objectType: { id: "10", name: "Laptop" }, attributes: [{ objectTypeAttributeId: "55", objectTypeAttribute: { id: "55", name: "Owner" }, objectAttributeValues: live[match[1]] ? [{ user: { accountId: live[match[1]] }, value: live[match[1]] }] : [] }] }); } if (match && options.method === "PUT") { puts.push(options); var vals = options.json.attributes[0].objectAttributeValues; live[match[1]] = vals.length ? String(vals[0].value) : ""; return Promise.resolve({}); } return Promise.reject(new Error("Unexpected request: " + options.method + " " + options.url)); }, renderHtmlPdf: function (html) { assert.ok(html.indexOf("Zmiany na stanie") >= 0 && html.indexOf("Stan po zmianie") >= 0); return Promise.resolve(pdf.renderTextPdf("protocol")); } });
        ...
        assert.strictEqual(live["1001"], ""); assert.strictEqual(live["1002"], "acc-1"); assert.strictEqual(live["1003"], "acc-1");
        reads.length = 0; puts.length = 0;
        ...
        assert.strictEqual(noChange.data.hasChanges, false); assert.strictEqual(service.requiresConfirmation(noChange), false); assert.strictEqual(puts.length, 0); assert.strictEqual(reads.length, 0, "No-change protocol must not snapshot mutation state.");
        console.log("Jira mixed per-asset protocol prepares PDF first, confirms later and skips no-change writes: OK");
