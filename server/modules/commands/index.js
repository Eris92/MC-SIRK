...
module.exports.createModule = function (context) {
    ...
    var catalog = {
        network: {
            key: "network", title: "Network", icon: "🌐", commands: [
                ...
                { id: "network-adapter-properties", label: "Network Settings", locales: { pl: { label: "Network Settings" }, en: { label: "Network Settings" } }, description: "Open properties for the adapter used by the preferred active default route.", type: 2, runAsUser: 2, elevatedUserSession: true, cmd: "$pickRoute={param($prefix);Get-NetRoute -DestinationPrefix $prefix -ErrorAction SilentlyContinue|Where-Object{$_.State -eq 'Alive'}|ForEach-Object{$routeCandidate=$_;$adapterCandidate=Get-NetAdapter -InterfaceIndex $routeCandidate.InterfaceIndex -ErrorAction SilentlyContinue;if($adapterCandidate -and $adapterCandidate.Status -eq 'Up'){[PSCustomObject]@{Route=$routeCandidate;Adapter=$adapterCandidate}}}|Sort-Object @{Expression={([long]$_.Route.RouteMetric)+([long]$_.Route.InterfaceMetric)};Ascending=$true},@{Expression={$_.Route.InterfaceMetric};Ascending=$true},@{Expression={$_.Route.RouteMetric};Ascending=$true},@{Expression={$_.Route.InterfaceIndex};Ascending=$true}|Select-Object -First 1};$selected=&$pickRoute '0.0.0.0/0';if(-not $selected){$selected=&$pickRoute '::/0'};if(-not $selected){throw 'No active default route with an Up adapter was found.'};$route=$selected.Route;$adapter=$selected.Adapter;$shell=New-Object -ComObject Shell.Application;$folder=$shell.Namespace(49);if(-not $folder){throw 'Network Connections shell folder is unavailable.'};$item=$folder.Items()|Where-Object{$_.Name -eq $adapter.Name}|Select-Object -First 1;if(-not $item){throw ('Network connection was not found: '+$adapter.Name)};$verbs=@($item.Verbs());$verb=$verbs|Where-Object{$name=($_.Name -replace '&','').Trim();$name -match '^(Properties|Właściwości)$'}|Select-Object -First 1;if(-not $verb){throw ('Properties/Właściwości verb was not found for network connection: '+$adapter.Name)};$verb.DoIt()" },
    ...
    function buildCommand(payload) {
        ...
        if (payload.commandId) {
            ...
            return { label: found.command.label, cmd: commandText, type: Number(found.command.type) || 1, runAsUser: Number(found.command.runAsUser) || 0, elevatedUserSession: found.command.elevatedUserSession === true };
        }
