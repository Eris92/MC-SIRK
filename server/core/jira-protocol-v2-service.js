...
module.exports.createJiraProtocolService = function (options) {
    options = options || {};
    var context = options.context, jiraAssets = options.jiraAssets;
    ...
    function meshUser(value) { var wanted = lower(value), web = shared.getWebServer(context.parent), users = web && web.users || {}, ids = Object.keys(users); for (var i = 0; i < ids.length; i++) { var user = users[ids[i]]; if (!user || user.deleted != null) continue; if (lower(user._id || ids[i]) === wanted || lower(user.name) === wanted || lower(shared.userName(user)) === wanted) return user; } return null; }
    function sharedDocument(title, body) { var templatePath = (context.nativePath || context.path).join(context.pluginRoot, "server", "templates", "document-a4.html"), template = context.fs.readFileSync(templatePath, "utf8"), logo = ""; try { var logoData = context.fs.readFileSync(branding.protocolLogoPath); if (logoData && logoData.length) logo = "data:image/png;base64," + logoData.toString("base64"); } catch (error) {} return template.replace(/{{DOCUMENT_TITLE}}/g, escape(title)).replace(/{{DOCUMENT_LOGO_DATA_URI}}/g, logo).replace(/{{DOCUMENT_LOGO_ALT}}/g, "Logo").replace(/{{DOCUMENT_BODY}}/g, body); }
    function renderPdf(html, fallbackText) { return Promise.resolve((options.renderHtmlPdf || htmlPdfRenderer.renderHtmlPdf)(html, { logoPath: branding.protocolLogoPath, fallbackText: fallbackText })).then(function (pdf) { if (!Buffer.isBuffer(pdf) || pdf.length < 100 || pdf.slice(0, 8).toString("ascii").indexOf("%PDF-1.") !== 0) throw new Error("PDF renderer returned an invalid artifact."); return pdf; }); }
    function rawText(data) { var lines = [data.hasChanges ? "PROTOKÓŁ ZMIAN SPRZĘTU" : "PROTOKÓŁ UZGODNIENIA STANU SPRZĘTU", "Użytkownik: " + data.user.name, "Osoba IT: " + data.itPerson.name, "", "Zmiany na stanie:"]; data.assets.forEach(function (asset) { lines.push("- " + asset.actionLabel + ": " + (asset.hostname || asset.assetIdentifier)); }); lines.push("", "Stan po zmianie:"); data.finalAssets.forEach(function (asset) { lines.push("- " + (asset.hostname || asset.assetIdentifier)); }); return lines.join("\n"); }
    function protocolInventory(userValue, variable, force) { return mutation.protocolInventory(userValue, variable, force); }
    ...
    function execute(script, payload, request) {
        ...
        return jiraAssets.listUsers(false).then(function (users) {
            ...
            updateProgress(id, 25, "Resolving Jira Assets", "running"); return protocolInventory(jiraUser.value, variable, false);
            ...
            changes = assets.filter(function (asset) { return asset.action !== "none"; }).map(function (asset) { return { assetId: asset.assetId, action: asset.action }; }); updateProgress(id, 40, changes.length ? "Capturing Jira ownership state" : "Building reconciliation", "running"); return changes.length ? mutation.snapshot(jiraUser, changes, variable) : { version: 1, user: shared.copy(jiraUser), changes: [] };
        }).then(function (value) {
            snapshot = value; var finalMap = Object.create(null); currentItems.forEach(function (asset) { var key = stableId(asset); if (key) finalMap[key] = publicAsset(asset, "none"); }); assets.forEach(function (asset) { if (asset.action === "return") delete finalMap[asset.assetId]; else if (asset.action === "receive") finalMap[asset.assetId] = publicAsset(asset, "none"); });
            ...
            updateProgress(id, 60, "Preparing protocol PDF", "running"); var fallback = rawText(data), html = documentRenderer.render(data, sharedDocument);
    ...
    function confirm(result, request) { result = object(result); var snapshot = result._jiraConfirmation; if (!snapshot || !Array.isArray(snapshot.changes) || !snapshot.changes.length) return Promise.resolve(shared.copy(result)); updateProgress(request && request.id, 100, "Finalizing Jira Assets", "confirming"); return mutation.apply(snapshot).then(function (summary) { var completed = shared.copy(result); delete completed._jiraConfirmation; completed.data.cmdb = { updated: summary.updated, assetIds: summary.assetIds }; completed.message = "Jira Assets updated after requester confirmation: " + summary.updated + " item(s)."; updateProgress(request && request.id, 100, "Ready", "ready"); return completed; }).catch(function (error) { updateProgress(request && request.id, 100, "Jira Assets finalization failed", "failed"); throw error; }); }

    return { execute: execute, confirm: confirm, requiresConfirmation: requiresConfirmation, progress: progress, protocolInventory: protocolInventory, isProtocolScript: protocolScript };
};
