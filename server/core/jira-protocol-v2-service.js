...
module.exports.createJiraProtocolService = function (options) {
    ...
    function execute(script, payload, request) {
        ...
        return jiraAssets.listUsers(false).then(function (users) {
            ...
            changes = assets.filter(function (asset) { return asset.action !== "none"; }).map(function (asset) { return { assetId: asset.assetId, action: asset.action }; }); updateProgress(id, 40, changes.length ? "Capturing Jira ownership state" : "Building reconciliation", "running"); return changes.length ? mutation.snapshot(jiraUser, changes) : { version: 1, user: shared.copy(jiraUser), changes: [] };
        }).then(function (value) {
