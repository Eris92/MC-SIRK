...
- `server/core/html-pdf-renderer.js` — owner preferowanego styled HTML -> PDF przez lokalny Chrome/Edge; każdy render używa osobnego zapisywalnego browser profile w bounded temp directory, a Edge zachowuje portable paths i bounded diagnostics bez wyłączania sandboxa;
- `server/core/jira-protocol-service.js` — jeden protocol lifecycle owner; po nieudanym styled browser renderze używa istniejącego dependency-free `pdf-text-renderer.js` dokładnie raz jako bounded fallback i nadal zapisuje artefakt dopiero po walidacji `%PDF-1.`;
...
```text
package.json -> 0.1.1-dev.67
config.json  -> 0.1.1-dev.67
```
...
Aktualne development notes: `docs/releases/0.1.1-dev.67.md`.
