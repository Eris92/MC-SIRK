...
- `public/shared/ui/parameter-dialog.js` — jeden owner natywnego MeshCentral parameter/confirmation dialog lifecycle; w Modern udany parameter submit jest finalizowany dopiero po hostowym `hidden.bs.modal`, aby kolejny dialog nie ścigał się z trwającym hide transition; checklist-backed `listMode`/`assetmulti` state jest utrzymywany przez hidden INPUT, a zwykłe select/user/asset pozostają SELECT;
...
```text
package.json -> 0.1.1-dev.55
config.json  -> 0.1.1-dev.55
```
...
Aktualne development notes: `docs/releases/0.1.1-dev.55.md`.
