...
- `server/core/jira-asset-service.js` — jeden server-side owner Jira users cache (24h freshness/stale fallback), Jira user options i dynamic Jira Assets options; generic script-owned Assets nadal wspierają bounded workspace-wide scan, compact cache v4, bounded concurrency, response attribute metadata i jawne Jira user/object references. Kanoniczny `Jira Asset Protocol` oraz `Jira Cache Assets` zachowują ostatni real-smoke-working scope z dev.64/dev.75: `objectType in objectTypeAndChildren("Sprzęt użytkownika")`; od `0.1.1-dev.79` plain assignment values z pól odpowiedzialności, w tym `Osoba_odpowiedzialna`, są rozpoznawane przez istniejący assignment-semantic classifier bez rozszerzania AQL; token nie trafia do cache;
...
```text
package.json -> 0.1.1-dev.81
config.json  -> 0.1.1-dev.81
```
...
Aktualne development notes: `docs/releases/0.1.1-dev.81.md`.
