(function () {
    ...
    function ensureStyle() {
        ...
        style.textContent = ".sirk-settings-root-menu{display:grid;gap:4px;width:100%}.sirk-settings-root-menu>.sirk-settings-root-button{width:100%;box-sizing:border-box;text-align:left}.sirk-settings-root-button.sirk-settings-root-active{background:var(--sirk-active-bg,rgba(77,107,216,.14))!important;color:var(--sirk-text,#172033)!important;font-weight:700!important;box-shadow:inset 3px 0 0 var(--sirk-active-accent,#4d6bd8)!important}.sirk-settings-primary-projected>summary{display:none!important}.sirk-settings-primary-projected>.sirk-settings-nav-group-body{display:block!important;padding:0!important}.sirk-settings-primary-projected{margin:0!important;padding:0!important;border:0!important;background:transparent!important}";
        (document.head || document.documentElement).appendChild(style);
    }

    function menu(primary, server) { var node = primary.querySelector(":scope > [data-settings-root-menu]"); if (!node) { node = document.createElement("div"); node.className = "sirk-settings-root-menu"; node.setAttribute("data-settings-root-menu", "1"); primary.insertBefore(node, server || null); } return node; }
    function rootButton(host, key, base) { var button = host.querySelector(':scope > [data-settings-root="' + key + '"]'); if (!button) { button = document.createElement("button"); button.type = "button"; button.className = "sirk-nav-item sirk-settings-root-button"; button.setAttribute("data-settings-root", key); button.addEventListener("click", function (event) { event.preventDefault(); event.stopPropagation(); activeRoot = key; needsDefaultSelection = true; if (active(base)) schedule(); else { base.click(); schedule(); } }); host.appendChild(button); } button.textContent = label(key); return button; }
    ...
    function apply() { var root = workspace(); if (!root) return; var primary = root.querySelector(":scope > .sirk-column-primary"); var secondary = root.querySelector(":scope > .sirk-column-secondary"); if (!primary || !secondary) return; ensureStyle(); var base = settingsButton(primary); var server = serverButton(primary); if (!base) return; base.hidden = true; base.style.display = "none"; base.setAttribute("aria-hidden", "true"); base.setAttribute("tabindex", "-1"); var host = menu(primary, server); var buttons = { modules: rootButton(host, "modules", base), portal: rootButton(host, "portal", base), integrations: rootButton(host, "integrations", base) }; if (server) primary.appendChild(server); Object.keys(buttons).forEach(function (key) { buttons[key].classList.toggle("sirk-settings-root-active", active(base) && key === activeRoot); buttons[key].setAttribute("aria-current", active(base) && key === activeRoot ? "page" : "false"); }); if (active(base)) project(secondary); else restore(secondary); }
