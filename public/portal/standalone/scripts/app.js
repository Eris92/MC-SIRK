(function () {
    ...
    function settings() {
        ...
        fetch(base + "/admin/settings", { credentials: "same-origin", cache: "no-store" }).then(function (response) { return response.json().then(function (value) { if (!response.ok) throw new Error(value.error || "Settings unavailable."); return value.value; }); }).then(function (snapshot) {
            Object.keys(snapshot.moduleSettings || {}).forEach(function (key) { var row = document.createElement("label"); row.className = "sirk-card"; var input = document.createElement("input"); input.type = "checkbox"; input.name = key; input.checked = snapshot.moduleSettings[key] && snapshot.moduleSettings[key].enabled === true; row.appendChild(input); row.appendChild(document.createTextNode(" " + key)); modules.appendChild(row); });
        ...
        form.addEventListener("submit", function (event) { event.preventDefault(); save.disabled = true; var values = {}; Array.prototype.forEach.call(form.querySelectorAll("input[name]"), function (input) { values[input.name] = input.checked; }); fetch(base + "/admin/settings", { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8" }, body: "payload=" + encodeURIComponent(JSON.stringify({ modules: values })) }).then(function (response) { return response.json().then(function (value) { if (!response.ok) throw new Error(value.error || "Save failed."); return value; }); }).then(function () { status.textContent = "Settings saved."; }).catch(function (error) { status.textContent = error.message || String(error); }).finally(function () { save.disabled = false; }); });
