(function () {
    ...
    function settings() {
        ...
        ["Overview", "Settings", "Plugins", "Server", "Debug", "System"].forEach(function (name, index) { var button = document.createElement("button"); button.type = "button"; button.className = "sirk-nav-item" + (index === 1 ? " active" : ""); button.textContent = name; button.onclick = function () { Array.prototype.forEach.call(primary.children, function (node) { node.classList.remove("active"); }); button.classList.add("active"); title.textContent = name; status.textContent = index === 1 ? "" : name + " jest zarządzane przez hosta MeshCentral."; form.hidden = index !== 1; modules.hidden = index !== 1; details.querySelectorAll(".sirk-settings-section").forEach(function (node) { node.hidden = index !== 1; }); if (index !== 1) secondary.innerHTML = "<p class=\"sirk-shared-muted\">Brak podkategorii.</p>"; }; primary.appendChild(button); });
