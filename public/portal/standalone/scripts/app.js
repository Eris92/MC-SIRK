(function () {
    ...
    function settings() {
        ...
        ["Overview", "Settings", "Plugins", "Server", "Debug", "System"].forEach(function (name, index) { var button = document.createElement("button"); button.type = "button"; button.className = "sirk-nav-item" + (index === 1 ? " active" : ""); button.textContent = name; button.onclick = function () { Array.prototype.forEach.call(primary.children, function (node) { node.classList.remove("active"); }); button.classList.add("active"); title.textContent = name; status.textContent = index === 1 ? "" : name + " jest zarządzane przez hosta MeshCentral."; form.hidden = index !== 1; modules.hidden = index !== 1; details.querySelectorAll(".sirk-settings-section").forEach(function (node) { node.hidden = index !== 1; }); secondary.innerHTML = "<p class=\"sirk-shared-muted\">" + (index === 1 ? "Wybierz kategorię ustawień." : "Overview · Status · Historia") + "</p>"; }; primary.appendChild(button); });
        ["Portal", "Moduły", "Integracje", "Uprawnienia folderów", "Diagnostyka"].forEach(function (name, index) { var button = document.createElement("button"); button.type = "button"; button.className = "sirk-nav-item" + (index === 0 ? " active" : ""); button.textContent = name; button.onclick = function () { Array.prototype.forEach.call(secondary.children, function (node) { node.classList.remove("active"); }); button.classList.add("active"); title.textContent = name; status.textContent = "Wybrano: " + name; }; secondary.appendChild(button); });
        ...
        fetch(base + "/admin/settings", { credentials: "same-origin", cache: "no-store" }).then(function (response) { return response.json().then(function (value) { if (!response.ok) throw new Error(value.error || "Settings unavailable."); return value.value; }); }).then(function (snapshot) {
            ...
            [["Portal", "Ustawienia przejścia między nowym i starym portalem oraz sesji."], ["Moduły", "Włączanie niezależnych modułów nowego portalu."], ["Integracje", "Stan połączeń z usługami zewnętrznymi (bez ujawniania sekretów)."], ["Uprawnienia", "Konfiguracja dostępu i uprawnień folderów."], ["Diagnostyka", "Ostatni stan diagnostyki i błędów API."], ["Aktualizacje", "Wersja portalu i stan wdrożenia."]].forEach(function (item) { var section = document.createElement("section"); section.className = "sirk-card sirk-settings-section"; var heading = document.createElement("h3"); heading.textContent = item[0]; section.appendChild(heading); var description = document.createElement("p"); description.className = "sirk-shared-muted"; description.textContent = item[1]; section.appendChild(description); sections.appendChild(section); });
