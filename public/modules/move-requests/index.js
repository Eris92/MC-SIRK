(function () {
    ...
    function normalizeNodeId(value) {
        if (value && typeof value === "object") {
            value = value._id || value.nodeid || value.nodeId || value.dbNodeKey || value.id;
        }
    ...
    function resolveHostNodeId(host) {
        ...
        add(window.SirkPlatformRuntime && window.SirkPlatformRuntime.state && window.SirkPlatformRuntime.state.nodeId);
        add(window.currentNodeId); add(window.xxcurrentNodeId); add(window.nodeid); add(window.xxnodeid);
        add(window.currentNode); add(window.currentDevice); add(window.selectedNode);
        ...
        try {
            ...
            add(params.get("gotonode")); add(params.get("nodeid"));
        } catch (error) {}
        try { return values.length ? decodeURIComponent(values[0]) : ""; }
        catch (error) { return values[0] || ""; }
    }

    function nodeName(nodeId) {
        if (window.currentNode && window.currentNode.name) return String(window.currentNode.name);
        if (window.nodes && window.nodes[nodeId] && window.nodes[nodeId].name) return String(window.nodes[nodeId].name);
        return String(nodeId || "Device");
    }

    function currentMeshId(nodeId) {
        if (window.currentNode && window.currentNode.meshid) return String(window.currentNode.meshid);
        if (window.nodes && window.nodes[nodeId] && window.nodes[nodeId].meshid) return String(window.nodes[nodeId].meshid);
        return "";
    }
    ...
    function setDialogStatus(status, state, message) {
        ...
        if (state && window.MeshThemeAdapter && typeof window.MeshThemeAdapter.status === "function") window.MeshThemeAdapter.status(status);
    }

    function openMoveDialog(nodeId) {
        nodeId = String(nodeId || "");
        if (!nodeId) { window.alert("No device is selected."); return; }
        ...
        module.api.api("meshes", { nodeId: nodeId }).then(function (result) {
            ...
            dialogFrame.className = "mc-move-dialog-frame";
            overlay.appendChild(dialogFrame);

            var dialog = document.createElement("div");
            dialog.className = "mc-move-dialog";
            dialogFrame.appendChild(dialog);
            ...
            (result.meshes || []).filter(function (mesh) { return !sourceMeshId || String(mesh.id) !== sourceMeshId; }).forEach(function (mesh) {
                var option = document.createElement("option"); option.value = mesh.id; option.textContent = mesh.name; select.appendChild(option);
            });
            ...
            submit.onclick = function () {
                ...
                if (!option) { setDialogStatus(status, "failed", "Select a target group."); return; }
                submitting = true; submit.disabled = true; setDialogStatus(status, "pending", "Submitting...");
                module.api.post("submit", { nodeId: nodeId, nodeName: nodeName(nodeId), sourceMeshId: sourceMeshId, sourceMeshName: sourceMeshName, targetMeshId: option.value, targetMeshName: option.textContent, note: note.value || "" }).then(function () {
                    submitting = false; submitted = true; submit.disabled = true; setDialogStatus(status, "completed", "Request sent.");
                }).catch(function (error) {
                    submitting = false; submitted = false; setDialogStatus(status, "failed", error.message || String(error)); submit.disabled = !select.options.length;
                });
            };
            actions.appendChild(submit);
            overlay.onclick = function (event) { if (event.target === overlay) closeDialog(overlay); };
            if (window.MeshThemeAdapter && typeof window.MeshThemeAdapter.refresh === "function") window.MeshThemeAdapter.refresh(overlay);
    ...
    function handleHostButtonClick(event) {
        if (event && event.preventDefault) event.preventDefault(); if (event && event.stopPropagation) event.stopPropagation();
        var host = document.getElementById("p10html") || document.getElementById("p10"); openMoveDialog(resolveHostNodeId(host)); return false;
    }
    function installHostButton() {
        removeElement(legacyTopButtonId);
        if (!hostButtonEnabled()) { removeElement(hostButtonId); return false; }
        var host = document.getElementById("p10html") || document.getElementById("p10"); if (!host) return false;
        ...
        if (existing && host.contains(existing)) {
            ...
            existing.disabled = false; existing.removeAttribute("onclick"); existing.removeAttribute("onmouseup"); existing.onclick = handleHostButtonClick; return true;
        }
        if (existing && existing.parentNode) existing.parentNode.removeChild(existing);
        var buttons = host.querySelectorAll('input[type="button"],button'); var anchor = null; var fallback = null;
        for (var index = 0; index < buttons.length; index++) {
            var value = buttonText(buttons[index]); fallback = buttons[index];
        ...
        anchor = anchor || fallback; if (!anchor || !anchor.parentNode) return false;
        var button = anchor.cloneNode(false); button.id = hostButtonId; button.type = "button";
        if (String(button.tagName).toLowerCase() === "input") button.value = "Move Request"; else button.textContent = "Move Request";
        button.title = "Submit a device move request"; button.disabled = false; button.setAttribute("data-meshcentral-plugin-pin", "SirkPlatform"); button.setAttribute("data-meshcentral-plugin-click", "Move Request host action"); button.removeAttribute("onclick"); button.removeAttribute("onmouseup"); button.onclick = handleHostButtonClick;
        anchor.parentNode.insertBefore(button, anchor.nextSibling); return true;
    }
    ...
    var module = window.SirkPlatformModuleShell.create({
        key: "moverequests", title: "Move Requests", menuTitle: "Move Requests", showInMenu: false, order: 120, preset: "standard",
        buttons: { favorites: false, manage: false, settings: false }, tabs: [{ key: "requests", title: "Requests" }], defaultTab: "requests",
