(function () {
    ...
    function openCommandEditor(shell, item) {
        shell.api("command-definition", { id: item.commandId }).then(function (response) {
            ...
            [1, 2, 3].forEach(function (level) { var option = document.createElement("label"), box = document.createElement("input"); box.type = "checkbox"; box.value = String(level); box.checked = (value.approvalLevels || []).indexOf(level) >= 0; option.appendChild(box); option.appendChild(document.createTextNode(" Level " + level)); approvals.appendChild(option); });
            ...
            card.appendChild(row(msg("Nazwa", "Label"), label)); card.appendChild(row(msg("Opis", "Description"), description)); card.appendChild(row("Approval", approvals)); card.appendChild(row(msg("Wykonanie", "Execution"), confirmLabel));
            ...
            save.onclick = function () { save.disabled = true; shell.post("command-definition", { id: item.commandId, label: label.value, description: description.value, approvalLevels: Array.prototype.map.call(approvals.querySelectorAll("input:checked"), function (box) { return Number(box.value); }), confirmExecution: confirm.checked }).then(function (result) { catalog = result.catalog || catalog; tree = buildTree(); tools.state.editMode = false; shell.render(); }).catch(function (error) { save.disabled = false; shell.error(host, error); }); };
