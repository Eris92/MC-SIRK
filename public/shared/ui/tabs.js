(function () {
    ...
    window.SharedTabs = {
        mount: function (options) {
            ...
            root.className = "mc-shared-tabs";
            var state = { active: options.active || "" };
            var buttons = {};

            function select(key, notify) {
                state.active = key;
                Object.keys(buttons).forEach(function (name) {
                    buttons[name].classList.toggle("active", name === key);
                });
            ...
            (options.tabs || []).forEach(function (tab) {
                ...
                item.type = "button";
                item.className = "btn btn-secondary btn-sm mc-shared-tab";
            ...
            var api = {
                ...
                setVisible: function (key, value) {
                    if (buttons[key]) buttons[key].hidden = value === false;
                }
            };
